...
window.mergeCells = function() {
    if (selectedCells.length < 2) {
        showTooltipMessage('Lütfen en az 2 hücre seçin!', 'warning');
        return;
    }
    
    // Sadece bitişik hücreleri birleştirebiliriz
    const cells = selectedCells.sort((a, b) => {
        const colA = a.id[0];
        const rowA = parseInt(a.id.substring(1));
        const colB = b.id[0];
        const rowB = parseInt(b.id.substring(1));
        
        if (colA !== colB) return colA.charCodeAt(0) - colB.charCodeAt(0);
        return rowA - rowB;
    });
    ...
    // Undo için kaydet
    saveUndoState('merge', firstCell, '', `MERGED:${firstCell.id}:${lastCell.id}`);
    
    // Ana hücreyi bul (ilk hücre)
    const mainCell = firstCell;
    
    // Diğer hücreleri gizle ve ana hücreye bağla
    for (let i = 1; i < cells.length; i++) {
        const cell = cells[i];
        cell.style.display = 'none';
        cell.dataset.mergedInto = mainCell.id;
        cell.classList.remove('selected', 'selected-range');
    }
    
    // Ana hücreyi birleştirilmiş olarak işaretle
    mainCell.classList.add('merged-cell');
    mainCell.classList.remove('selected-range');
    mainCell.classList.add('selected');
    mainCell.dataset.mergedCells = cells.map(c => c.id).join(',');
    mainCell.dataset.mergeRange = `${firstCell.id}:${lastCell.id}`;
    
    // Eğer ana hücrede değer yoksa ama diğer hücrelerde varsa, ilk değeri al
    if (!mainCell.value && cells.some(c => c.value)) {
        const cellWithValue = cells.find(c => c.value);
        if (cellWithValue) {
            mainCell.value = cellWithValue.value;
            if (cellWithValue.dataset.originalValue) {
                mainCell.dataset.originalValue = cellWithValue.dataset.originalValue;
            }
        }
    }
    
    // Seçimi güncelle
    selectedCells = [mainCell];
    selectedCell = mainCell;
    
    showTooltipMessage(`${cells.length} hücre birleştirildi!`, 'success');
};

window.unmergeCells = function() {
    if (!selectedCell || !selectedCell.classList.contains('merged-cell')) {
        showTooltipMessage('Lütfen birleştirilmiş bir hücre seçin!', 'warning');
        return;
    }
    
    const mainCell = selectedCell;
    const mergedCells = mainCell.dataset.mergedCells ? mainCell.dataset.mergedCells.split(',') : [];
    
    if (mergedCells.length < 2) {
        showTooltipMessage('Bu hücre birleştirilmemiş!', 'warning');
        return;
    }
    
    // Undo için kaydet
    saveUndoState('unmerge', mainCell, mainCell.value, '');
    
    // Tüm hücreleri görünür yap
    mergedCells.forEach(cellId => {
        const cell = document.getElementById(cellId);
        if (cell) {
            cell.style.display = '';
    ...
    // Ana hücreden merged özelliklerini kaldır
    delete mainCell.dataset.mergedCells;
    delete mainCell.dataset.mergeRange;
    mainCell.classList.remove('merged-cell');
    
    showTooltipMessage('Hücre birleştirmesi kaldırıldı!', 'success');
};
